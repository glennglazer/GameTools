...
"""
File: skyrim_parse_perks_to_json.py
Author: Glenn Glazer

Parse the pipe-delimited perks raw text file produced by the scraper into JSON,
...
Diff files (<stem>.upsert.json, <stem>.delete.json) follow the same pattern
as the alchemy ingredients pipeline.
"""
...
_PARSE_DIR = _SCRIPT_DIR.parent / 'perks_parse'
_DEFAULT_INFILE = str(_PARSE_DIR / 'skyrim_alchemy_perks_raw.txt')
_DEFAULT_OUTFILE = str(_SCRIPT_DIR / 'skyrim_alchemy_perks.json')

EXPECTED_FIELDS = 4


def parse(infile: str) -> list:
    """Read pipe-delimited raw file and return list of perk dicts."""
    try:
    ...
    for lineno, line in enumerate(lines, 1):
        ...
        name, skill_level_str, prerequisite, description = parts
        try:
            skill_level = int(skill_level_str)
        except ValueError:
            raise ValueError(f"Line {lineno}: skill_level is not an integer: {skill_level_str!r}")
        perks.append({
            'name': name.strip(),
...
def write_diff_files(outfile: str, upsert: list, delete: list) -> None:
    ...
    try:
        ...
        print(f"Failed to write diff files for {Path(outfile).name}: {e}", file=sys.stderr)
        raise


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Parse Skyrim alchemy perks raw text into JSON.'
    )
    parser.add_argument('infile', nargs='?', default=_DEFAULT_INFILE,
                        help=f'pipe-delimited perks raw file (default: {_DEFAULT_INFILE})')
    parser.add_argument('outfile', nargs='?', default=_DEFAULT_OUTFILE,
                        help=f'output JSON file (default: {_DEFAULT_OUTFILE})')
