...
"""
File: create_or_update_skyrim_alchemy_perks.py
Author: Glenn Glazer

Create or incrementally update the skyrim_alchemy_perks table from diff files
produced by skyrim_parse_perks_to_json.py.

Exits immediately (returncode 0) if no diff files are present.
Halts with a non-zero exit code on any database error.

Table schema: skyrim_alchemy_perks
...
TABLE_NAME = 'skyrim_alchemy_perks'
KEY_COL = 'name'
INDEX_NAME = 's_ap_name'
GAME_LABEL = 'Skyrim alchemy perks'

_SCRIPT_DIR = Path(__file__).parent.resolve()
_FAMILY_ROOT = _SCRIPT_DIR.parent.parent.parent
_JSON_DIR = _SCRIPT_DIR.parent / 'perks_json'
_DEFAULT_JSON_FILE = str(_JSON_DIR / 'skyrim_alchemy_perks.json')
...
if __name__ == '__main__':
    ...
    parser.add_argument('json_file', nargs='?', default=_DEFAULT_JSON_FILE,
                        help=f'perks JSON file (default: {_DEFAULT_JSON_FILE})')
