#!/usr/bin/python3
"""
Create or incrementally update the skyrim_smithing_improvement table.

Table schema: skyrim_smithing_improvement
  quality             TEXT (unique key)
  skill_without_perk  INTEGER
  skill_with_perk     INTEGER
  armor_effect        TEXT
  weapon_effect       TEXT
"""
...
TABLE_NAME = 'skyrim_smithing_improvement'
KEY_COL = 'quality'
INDEX_NAME = 's_si_quality'
GAME_LABEL = 'Skyrim smithing improvement'

_SCRIPT_DIR = Path(__file__).parent.resolve()
_REPO_ROOT = _SCRIPT_DIR.parent.parent.parent.parent
_JSON_DIR = _SCRIPT_DIR.parent / 'improvement_json'
_DEFAULT_JSON_FILE = str(_JSON_DIR / 'skyrim_smithing_improvement.json')
...
def apply_deletes(cur, table_name: str, delete_data: list, key_col: str = 'quality') -> None:
    cur.executemany(
...
def apply_upserts(conn, table_name: str, upsert_data: list, key_col: str = 'quality') -> None:
    cur = conn.cursor()
    cur.executemany(
        f'DELETE FROM {table_name} WHERE {key_col} = ?',
