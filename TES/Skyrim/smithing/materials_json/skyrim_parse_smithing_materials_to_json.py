#!/usr/bin/python3
"""
Parse the pipe-delimited smithing crafting materials raw text file into JSON,
and write diff files for the SQL loader.

Input format (2 fields, one line per category-material pair):
...
_DEFAULT_INFILE = str(_PARSE_DIR / 'skyrim_smithing_materials_raw.txt')
_DEFAULT_OUTFILE = str(_SCRIPT_DIR / 'skyrim_smithing_materials.json')
...
def parse(infile: str) -> list:
    ...
    for lineno, line in enumerate(lines, 1):
        ...
        if not category:
            raise ValueError(f'Line {lineno}: empty smithing_category')
        if not material:
            raise ValueError(f'Line {lineno}: empty crafting_material')
        rows.append({
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Parse Skyrim smithing crafting materials raw text into JSON.')
    ...
    if not new_data:
        print('No material rows parsed — check raw file.', file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        upsert, delete = compute_diff(
            old_data, new_data,
            lambda r: (r['smithing_category'], r['crafting_material'])
        )
