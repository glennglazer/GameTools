#!/usr/bin/python3
"""
Parse disenchant_apparel_raw.json (produced by skyrim_scrape_disenchant.py) into
the canonical output JSON and diff files consumed by the SQL loader.

Input:  disenchant_apparel_raw.json  — list of {effect, item, note} records
Output: disenchant_apparel.json      — same list (canonical snapshot)
        disenchant_apparel.upsert.json / .delete.json  — diff for SQL loader
"""
...
_DEFAULT_INFILE = str(_PARSE_DIR / 'disenchant_apparel_raw.json')
_DEFAULT_OUTFILE = str(_SCRIPT_DIR / 'disenchant_apparel.json')

REQUIRED_KEYS = ('effect', 'item', 'note')


def parse(infile: str) -> list:
    """Load and validate disenchant apparel records from raw JSON."""
    try:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Parse Skyrim disenchant apparel raw JSON into output JSON.'
    )
