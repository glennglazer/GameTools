#!/usr/bin/python3
"""
Parse the pipe-delimited enchantment effects raw text file into JSON,
and write diff files for the SQL loader.

Input format (one line per effect):
  name|school
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Parse Skyrim enchantment effects raw text into JSON.'
    )
    ...
    if not new_data:
        print('No effects parsed — check raw file.', file=sys.stderr)
        sys.exit(1)
