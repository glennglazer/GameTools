#!/usr/bin/python3
"""
Create or incrementally update the skyrim_enchant_souls table.

Table schema: skyrim_enchant_souls
  creature   TEXT
  soul_size  TEXT
  (composite unique key on (creature, soul_size))
"""
...
INDEX_NAME = 's_es_creature_size'
GAME_LABEL = 'Skyrim enchanting creature souls'

_SCRIPT_DIR = Path(__file__).parent.resolve()
_REPO_ROOT = _SCRIPT_DIR.parent.parent.parent.parent
_JSON_DIR = _SCRIPT_DIR.parent / 'creature_souls_json'
_DEFAULT_JSON_FILE = str(_JSON_DIR / 'skyrim_enchant_souls.json')
...
def apply_deletes(cur, table_name: str, delete_data: list) -> str:
    sql = f'DELETE FROM {table_name} WHERE creature = ? AND soul_size = ?'
    cur.executemany(sql, [(r['creature'], r['soul_size']) for r in delete_data])
    return sql
...
def apply_upserts(conn, table_name: str, upsert_data: list) -> None:
    cur = conn.cursor()
    cur.executemany(
        f'DELETE FROM {table_name} WHERE creature = ? AND soul_size = ?',
        [(r['creature'], r['soul_size']) for r in upsert_data],
    )
    conn.commit()
...
if __name__ == '__main__':
    ...
    try:
        ...
        if table_exists is None:
            ...
            current_sql = (
                f'CREATE UNIQUE INDEX {INDEX_NAME} ON {TABLE_NAME} (creature, soul_size)'
            )
            ...
            if delete_data:
                current_sql = f'DELETE FROM {TABLE_NAME} WHERE creature = ? AND soul_size = ?'
                apply_deletes(cur, TABLE_NAME, delete_data)
                conn.commit()
                if args.verbose:
                    print(f'Deleted {len(delete_data)} rows from {TABLE_NAME}.')
            if upsert_data:
                current_sql = 'DELETE+INSERT skyrim_enchant_souls WHERE creature=? AND soul_size=?'
