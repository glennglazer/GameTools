#!/usr/bin/python3
"""
Create or incrementally update the skyrim_enchant_apparel table.

Table schema: skyrim_enchant_apparel
  enchantment  TEXT (unique key)
  head         INTEGER (boolean)
  chest        INTEGER (boolean)
  hands        INTEGER (boolean)
  feet         INTEGER (boolean)
...
TABLE_NAME = 'skyrim_enchant_apparel'
KEY_COL = 'enchantment'
INDEX_NAME = 's_ea_ench'
GAME_LABEL = 'Skyrim enchanting apparel'

_SCRIPT_DIR = Path(__file__).parent.resolve()
_REPO_ROOT = _SCRIPT_DIR.parent.parent.parent.parent
_JSON_DIR = _SCRIPT_DIR.parent / 'enchant_apparel_json'
_DEFAULT_JSON_FILE = str(_JSON_DIR / 'skyrim_enchant_apparel.json')
...
def apply_deletes(cur, table_name: str, delete_data: list, key_col: str = 'enchantment') -> str:
    sql = f'DELETE FROM {table_name} WHERE {key_col} = ?'
    cur.executemany(sql, [(r[key_col],) for r in delete_data])
    return sql


def apply_upserts(conn, table_name: str, upsert_data: list, key_col: str = 'enchantment') -> None:
    cur = conn.cursor()
    cur.executemany(
