...
The scripts in this directory incrementally update the Morrowind alchemy tables in the shared
...
- `create_or_update_morrowind_alchemy_ingredients.py` — manages the `morrowind_alchemy_ingredients` table
- `create_or_update_morrowind_alchemy_effects.py` — manages the `morrowind_alchemy_effects` table

## How it works

Each script expects a main JSON file (e.g. `morrowind_all_ingredients.json`) and looks for two
...
python3 create_or_update_morrowind_alchemy_ingredients.py
python3 create_or_update_morrowind_alchemy_effects.py

# Override paths explicitly
python3 create_or_update_morrowind_alchemy_ingredients.py \
    /abs/path/to/morrowind_all_ingredients.json \
