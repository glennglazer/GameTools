...
The scripts in this directory incrementally update the Oblivion alchemy tables in the shared
...
- `create_or_update_oblivion_alchemy_ingredients.py` — manages the `oblivion_alchemy_ingredients` table
- `create_or_update_oblivion_alchemy_effects.py` — manages the `oblivion_alchemy_effects` table

## How it works

Each script expects a main JSON file (e.g. `oblivion_all_ingredients.json`) and looks for two
...
python3 create_or_update_oblivion_alchemy_ingredients.py
python3 create_or_update_oblivion_alchemy_effects.py

# Override paths explicitly
python3 create_or_update_oblivion_alchemy_ingredients.py \
    /abs/path/to/oblivion_all_ingredients.json \
