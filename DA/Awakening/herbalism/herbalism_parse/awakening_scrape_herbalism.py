"""Scrape DAO:A Herbalism recipe and effects data from the Fandom wiki.

Fetches wikitext for all Awakening herbalism recipe pages from the wiki category,
plus the "Dragon Age: Origins - Awakening" crafted-items subsection of the main
Herbalism page, and saves everything as a single raw JSON file.

There is no dedicated Awakening locations section on the Herbalism wiki page, so
locations_wikitext is saved as an empty string (supply table will have 0 rows).

Usage:
    python3 awakening_scrape_herbalism.py <output_raw_json>
"""
import argparse
import json
import sys
...
CATEGORY = "Category:Dragon_Age:_Origins_-_Awakening_Herbalism_recipes"
HERBALISM_PAGE = "Herbalism"
CRAFTED_ITEMS_SECTION = 5  # "Herbalism Crafted Items → Dragon Age: Origins - Awakening"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Scrape DAO:A Herbalism data from the Fandom wiki"
    )
    ...
    # Exclude the "Herbalism recipes" navigation page (no "Recipe" suffix)
    ...
    # ── 3. Fetch the Awakening crafted-items section ──────────────────────────
    print("Fetching Awakening crafted items section from main Herbalism page...")
    ...
    raw = {
        ...
        # No Awakening-specific supply section exists on the Herbalism wiki page.
        "locations_wikitext": "",
    }
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(raw, f, indent=2, ensure_ascii=False)

    print(f"\nSaved {len(recipes)} recipes + crafted items wikitext → {out_path}")
