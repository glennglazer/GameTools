"""Load DAO:A Poison-Making (poisons + grenades) JSON records into the DAO:A SQLite database.

Creates or upserts five tables:
  awakening_poisons_grenades_recipes          — wide recipe table with type column
  awakening_poisons_grenades_ingredient_recipes — ingredient → recipe reverse-lookup
  awakening_poisons_grenades_tiers            — recipe → required tier (all 4)
  awakening_poisons_grenades_unlimited_supply — vendor/location (empty for Awakening)
  awakening_poisons_grenades_effects          — item name → damage_type/power/effect

Usage:
    python3 create_or_update_awakening_poisons_grenades.py \\
...
# 2 parent calls: poisons_grenades_sql/ → poisons_grenades/ → Awakening/
_FAMILY_ROOT = _SCRIPT_DIR.parent.parent
_JSON_DIR = _SCRIPT_DIR.parent / "poisons_grenades_json"

_DEFAULT_RECIPES = str(_JSON_DIR / "awakening_poisons_grenades_recipes.json")
_DEFAULT_ING_REC = str(_JSON_DIR / "awakening_poisons_grenades_ingredient_recipes.json")
_DEFAULT_TIERS   = str(_JSON_DIR / "awakening_poisons_grenades_tiers.json")
_DEFAULT_SUPPLY  = str(_JSON_DIR / "awakening_poisons_grenades_unlimited_supply.json")
_DEFAULT_EFFECTS = str(_JSON_DIR / "awakening_poisons_grenades_effects.json")
_DEFAULT_DB      = str(_FAMILY_ROOT / "database" / "gametools.sqlite3")

RECIPES_TABLE = "awakening_poisons_grenades_recipes"
ING_REC_TABLE = "awakening_poisons_grenades_ingredient_recipes"
TIERS_TABLE   = "awakening_poisons_grenades_tiers"
SUPPLY_TABLE  = "awakening_poisons_grenades_unlimited_supply"
EFFECTS_TABLE = "awakening_poisons_grenades_effects"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Load DAO:A Poison-Making data into the DAO:A GameTools SQLite database"
    )
    ...
    print("Starting database update for DAO:A Poisons & Grenades")
    ...
    conn.close()
    print("Database update complete for DAO:A Poisons & Grenades (5 tables).")
