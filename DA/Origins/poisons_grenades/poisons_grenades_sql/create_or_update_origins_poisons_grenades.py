"""Load DAO Poison-Making (poisons + grenades) JSON records into the GameTools SQLite database.

Creates or upserts five tables:
  origins_poisons_grenades_recipes          — wide recipe table with type column
  origins_poisons_grenades_ingredient_recipes — ingredient → recipe reverse-lookup
  origins_poisons_grenades_tiers            — recipe → required tier (1-4)
  origins_poisons_grenades_unlimited_supply — vendor/location for unlimited ingredient sources
  origins_poisons_grenades_effects          — item name → damage_type/power/effect

Usage:
    python3 create_or_update_origins_poisons_grenades.py \\
...
_FAMILY_ROOT = _SCRIPT_DIR.parent.parent.parent   # poisons_grenades_sql/ → poisons_grenades/ → Origins/ → DA/
_JSON_DIR = _SCRIPT_DIR.parent / "poisons_grenades_json"

_DEFAULT_RECIPES = str(_JSON_DIR / "origins_poisons_grenades_recipes.json")
_DEFAULT_ING_REC = str(_JSON_DIR / "origins_poisons_grenades_ingredient_recipes.json")
_DEFAULT_TIERS = str(_JSON_DIR / "origins_poisons_grenades_tiers.json")
_DEFAULT_SUPPLY = str(_JSON_DIR / "origins_poisons_grenades_unlimited_supply.json")
_DEFAULT_EFFECTS = str(_JSON_DIR / "origins_poisons_grenades_effects.json")
_DEFAULT_DB = str(_FAMILY_ROOT / "database" / "gametools.sqlite3")

RECIPES_TABLE = "origins_poisons_grenades_recipes"
ING_REC_TABLE = "origins_poisons_grenades_ingredient_recipes"
TIERS_TABLE = "origins_poisons_grenades_tiers"
SUPPLY_TABLE = "origins_poisons_grenades_unlimited_supply"
EFFECTS_TABLE = "origins_poisons_grenades_effects"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Load DAO Poison-Making data into the GameTools SQLite database"
    )
    ...
    print("Starting database update for DAO Poisons & Grenades")
    ...
    conn.close()
    print("Database update complete for DAO Poisons & Grenades (5 tables).")
