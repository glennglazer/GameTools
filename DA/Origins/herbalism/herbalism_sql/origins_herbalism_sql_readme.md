...
Loads the four herbalism JSON files into the GameTools SQLite database (`DA/database/gametools.sqlite3`), creating tables on first run and upserting on subsequent runs.
...
| `origins_herbalism_ingredient_recipes` | `(ingredient, recipe)` | ~65 | Ingredient → recipe reverse-lookup join table |
