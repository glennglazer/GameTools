...
class TestInitialLoad:
    ...
    def test_type_column_poison(self, db):
        conn = sqlite3.connect(db)
        t = conn.execute(
    ...
    def test_type_column_grenade(self, db):
        conn = sqlite3.connect(db)
        t = conn.execute(
    ...
    def test_filter_by_type_grenade(self, db):
        conn = sqlite3.connect(db)
        rows = conn.execute(
    ...
    def test_filter_by_type_poison(self, db):
        conn = sqlite3.connect(db)
        rows = conn.execute(
    ...
    def test_supply_note_stored(self, db):
        conn = sqlite3.connect(db)
        note = conn.execute(
...
class TestQueryPatterns:
    ...
    def test_grenade_tier_all_two(self, db):
...
class TestPGEffects:
    ...
    def test_nature_damage_type(self, db):
        conn = sqlite3.connect(db)
        row = conn.execute(
    ...
    def test_fire_damage_type(self, db):
        conn = sqlite3.connect(db)
        row = conn.execute(
    ...
    def test_mana_drain_damage_type(self, db):
        conn = sqlite3.connect(db)
        row = conn.execute(
    ...
    def test_stamina_drain_damage_type(self, db):
        conn = sqlite3.connect(db)
        row = conn.execute(
    ...
    def test_null_effect_stored(self, db):
        conn = sqlite3.connect(db)
        effect = conn.execute(
    ...
    def test_no_duplicates_on_reload(self, db):
        """Upsert with same data is idempotent on the shared DB."""
        import pandas as pd, sqlite3 as _sqlite3
        conn = _sqlite3.connect(db)
        cur = conn.cursor()
        loader.upsert_by_key(conn, cur, pd.DataFrame(EFFECTS), loader.EFFECTS_TABLE, "name")
